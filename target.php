...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        try {
            $stmt = $conn->query("SELECT MAX(SNO) AS max_SNO FROM target");
            $row = $stmt->fetch();
            $next_SNO = $row['max_SNO'] ? $row['max_SNO'] + 1 : 1;

            $stmt = $conn->prepare("INSERT INTO target (SNO, LFNO, DrawingNo, Description, Variant, Target, Year) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$next_SNO, $LFNO, $DrawingNo, $Description, $Variant, $Target]);
            $conn->exec("
            ...
            $conn->exec("
                ...
                SET BAL = TGT - CLS
                WHERE SNO IN (SELECT SNO FROM operationdetails);
            ");
        ...
        if (!empty($delete_ids)) {
            try {
                ...
                $stmt = $conn->prepare("DELETE FROM target WHERE SNO IN ($placeholders)");
                $stmt->execute($delete_ids);

                // Reindex SNO after deletion
                $conn->exec("SET @new_SNO = 0;");
                $conn->exec("UPDATE target SET SNO = (@new_SNO := @new_SNO + 1) ORDER BY SNO");
        ...
        foreach ($modified_data as $row) {
            try {
                $stmt = $conn->prepare("UPDATE target SET LFNO = ?, DrawingNo = ?, Description = ?, Variant = ?, Target = ? WHERE SNO = ?");
                $stmt->execute([$row['LFNO'], $row['DrawingNo'], $row['Description'], $row['Variant'], $row['Target'], $row['SNO']]);
            } catch (PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
        }
        ...
        try {
            // Update operationdetails.TGT and BAL
            $conn->beginTransaction();
            $conn->exec("
                UPDATE operationdetails od
                JOIN target t ON od.DrawingNo = t.DrawingNo
...
try {
    $stmt = $conn->query("SELECT * FROM target ORDER BY SNO");
    $rows = $stmt->fetchAll();
} catch (PDOException $e) {
    echo "Error: " . $e->getMessage();
}

// Fetch user details (assuming user is logged in and user_id is 1 for this example)
$current_user_id = 1;
try {
...
<head>
    ...
    <title>Annual Target 2024</title>
    <style>
          body {
...
function toggleEditable() {
    const tableCells = document.querySelectorAll("td.LFNO, td.DrawingNo, td.Description, td.Variant, td.Target");
...
function submitModifications() {
    ...
    modifiedRows.forEach((sno) => {
        ...
        data.push({ SNO: sno, LFNO, DrawingNo, Description, Variant, Target });
    });
    ...
    <a href="?logout=true" class="btn" style="width: auto; display: inline-block; background-color: red;">Logout</a>
        ...
        <table>
            ...
            <tbody>
                ...
                <td class="Target" contenteditable="false"><?php echo $row['Target']; ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
